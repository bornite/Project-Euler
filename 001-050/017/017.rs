fn to_word_under10(n: u64) -> String {
    match n {
        0 => "zero".to_string(),
        1 => "one".to_string(),
        2 => "two".to_string(),
...
fn to_word_under20(n: u64) -> String {
    assert!(n < 20);
    if n < 10 {
...
fn to_word_under100(n: u64) -> String {
    assert!(n < 100);
    if n < 20 {
...
fn to_word_under1000(n: u64) -> String {
    assert!(n < 1000);
    if n < 100 {
...
fn count_letters(max: u64) -> u64 {
    (1..max + 1)
        .map(to_word)
        .map(|w| w.chars().filter(|&c| c != '-' && c != ' ').count() as u64)
        .sum()
}

fn main () {
    println!("{}", count_letters(1000));
}